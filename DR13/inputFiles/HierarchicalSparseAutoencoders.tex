...
\end{equation}
\commentJP{The regularizer does not make sense; it must be scalar.  Do you mean the sum of the Frobenius norms of the two $W_i$?}
...
\end{equation}
 ...
 \begin{center}
  ...
  \end{tabular}
\end{center}
 \caption{Statistics of parts at Layers 1-5 in the 2D and view-based 3D hierarchies. \commentJP{What is $Err$; is it $\textrm{avg}_j r_j^l$? Be very clear.}}
 \label{tab:stats}
\end{table}
...
As there are many-to-many correspondences, choosing the best part according to the conditional probability is not always the best choice, but only in overall 20\% \commentJP{???} of the cases, due to ambiguity of edge structures which could correspond to many depth parts. Therefore, one solution for effective inference of view-based 3D parts, given observed 2D parts, consists of extracting complementary information from the region surrounding an edge part using HOG features. Additionally, we trained a $k$-nearest-neighbour classifier to improve the prediction of view-based 3D parts. The results obtained for each layer are presented in Table~\ref{tab:stats.inference} together with the reconstruction error of view-based 3D parts given 2D parts, and statistical measures such as conditional entropy. The cross-hierarchy reconstruction error is computed using an adapted version of Eqn.~\ref{eqn:recon-error.patch},
\begin{equation}
...
\end{equation}
 ...
 \begin{center}
  \begin{tabular}{ | c | c | c | c | c | c | c |r |}
      \hline
      Layer $l$  & max. prediction & $Err(3D|2D)$ & $k$-NN & $Err(3D|2D)$ & $H(\Omega_*^l|\Gamma^l)$ & $H(\Gamma^l|\Omega_*^l)$  \\ \hline
  ...
  \end{tabular}
\end{center}
 \caption{Statistics of inference results of view-based 3D parts given 2D observed parts at Layers 1-5 in the 2D and view-based 3D hierarchies. \commentJP{What exactly does the first column show?  See also my comments on Table~\ref{tab:stats}.}}
