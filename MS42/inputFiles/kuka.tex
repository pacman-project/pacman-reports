\subsection{Software: Kuka LWR4+ package}
\label{sec:kuka}
...
Exploiting the external torque command $\tau_{FRI}$ in \eqref{eq:axisspecificstiffnesscontroller}, a new set of ROS-enabled controllers has been developed in \textit{C++} language\footnote{Online available at \url{https://github.com/CentroEPiaggio/kuka-lwr/tree/master/lwr_controllers}}, following some interesting paradigms available in the robotics literature.
...
A classical approach for manipulators control is to define a task in the joint space, i.e.\ providing a motion law for specific joints to be followed. An alternative way to define the motion law in task space is the \textit{Operational Space Formulation} \cite{khatib87}, in which the desired behavior of the robot can be described through the dynamics of its end-effector. This approach allows to formulate tasks in a more human-like way, where the importance is put more on the manipulation task rather than on the internal behaviour of the robot. Based on this, the following list of controllers are implemented.
...
\dot{q} = J^+\dot{x}
\end{equation}
where $J^+$ is a generalized inverse of the Jacobian. For the project's purpose, it has been implemented as the Moore-Penrose damped pseudo-inverse through the SVD method, denoted in the sequel as $J^{\dagger}$.\\
